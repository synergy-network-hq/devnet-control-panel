...
6. Install the Network Node Monitor App (macOS, Linux, Windows)
...
The Node Monitor app is a Tauri desktop application (Rust backend + React frontend).

### 2.1 Workspace locations by OS

The app writes runtime state under `monitor-workspace`.

- **macOS (current)**: `~/.synergy-node-monitor/monitor-workspace`
- **Linux (current)**: `~/.synergy-node-monitor/monitor-workspace`
- **Windows (current)**: `%USERPROFILE%\.synergy-node-monitor\monitor-workspace`

Legacy path (macOS builds prior to this fix):

- `~/Library/Application Support/com.synergy.node-monitor/monitor-workspace`

The app now prefers the no-space workspace path and migrates legacy workspace data when the new workspace is empty.
...
- Rendered configs: `devnet/lean15/configs/machine-*.toml`
...
### 4.3 Authoritative 15-node machine map

| Machine | Role Group | Role | Node Type | VPN IP | P2P | RPC | WS | gRPC | Discovery | Auto Validator | Pruning | VRF |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| machine-01 | consensus | validator | validator | 10.50.0.1 | 38638 | 48638 | 58638 | 50051 | 39638 | true | false | true |
| machine-02 | consensus | validator | validator | 10.50.0.2 | 38639 | 48639 | 58639 | 50052 | 39639 | true | false | true |
| machine-03 | consensus | validator | validator | 10.50.0.3 | 38640 | 48640 | 58640 | 50053 | 39640 | true | false | true |
| machine-04 | consensus | validator | validator | 10.50.0.4 | 38641 | 48641 | 58641 | 50054 | 39641 | true | false | true |
| machine-05 | consensus | validator | validator | 10.50.0.5 | 38642 | 48642 | 58642 | 50055 | 39642 | true | false | true |
| machine-06 | interop | relayer | relayer | 10.50.0.6 | 38643 | 48643 | 58643 | 50056 | 39643 | false | true | false |
| machine-07 | interop | cross-chain-verifier | cross-chain-verifier | 10.50.0.7 | 38644 | 48644 | 58644 | 50057 | 39644 | false | true | false |
| machine-08 | interop | oracle | oracle | 10.50.0.8 | 38645 | 48645 | 58645 | 50058 | 39645 | false | true | false |
| machine-09 | interop | witness | witness | 10.50.0.9 | 38646 | 48646 | 58646 | 50059 | 39646 | false | true | false |
| machine-10 | governance | committee | committee | 10.50.0.10 | 38647 | 48647 | 58647 | 50060 | 39647 | true | false | true |
| machine-11 | governance | security-council | security-council | 10.50.0.11 | 38648 | 48648 | 58648 | 50061 | 39648 | true | false | true |
| machine-12 | pqc | pqc-crypto | pqc-crypto | 10.50.0.12 | 38649 | 48649 | 58649 | 50062 | 39649 | false | true | false |
| machine-13 | services | rpc-gateway | rpc-gateway | 10.50.0.13 | 38650 | 48650 | 58650 | 50063 | 39650 | false | true | false |
| machine-14 | services | indexer | indexer | 10.50.0.14 | 38651 | 48651 | 58651 | 50064 | 39651 | false | false | false |
| machine-15 | services | observer | observer | 10.50.0.15 | 38652 | 48652 | 58652 | 50065 | 39652 | false | false | false |
...
| machine-14 | 10.50.0.14 | 51833 |
| machine-15 | 10.50.0.15 | 51834 |
...
1. Decide machine mapping (`machine-01` to `machine-15`) and assign hosts.
...
4. Confirm UDP path between hosts for WG listen ports (`51820-51834`).
5. Confirm all hosts reserve the `10.50.0.0/24` tunnel subnet.

### 5.2.1 WireGuard install commands by OS

Run on each node host before Node Monitor install/provisioning.
...
2. Apply only `machine-01.conf` to first node.
...
```bash
# operator machine
WORKSPACE="${HOME}/.synergy-node-monitor/monitor-workspace"
cd "$WORKSPACE"
...
ls devnet/lean15/wireguard/configs/machine-01.conf

# 5) if machine-01 is remote, copy config to that host
scp devnet/lean15/wireguard/configs/machine-01.conf <user>@<machine-01-host>:/tmp/synergy-devnet.conf
...
```bash
WORKSPACE="${HOME}/.synergy-node-monitor/monitor-workspace"
sudo install -m 600 "$WORKSPACE/devnet/lean15/wireguard/configs/machine-01.conf" /etc/wireguard/synergy-devnet.conf
...
| Node 1 config file | `machine-01.conf` | Deterministic first validator |
...
| First role brought online | `validator` (`machine-01`) | Establishes first consensus participant |
...
- App bundle: `tools/devnet-control-panel/src-tauri/target/release/bundle/macos/Synergy Node Monitor.app`
...
```bash
xattr -dr com.apple.quarantine "/Applications/Synergy Node Monitor.app"
```
...
- Installer bundles present: `devnet/lean15/installers/machine-01`..`machine-15`
...
1. validators (`machine-01`..`machine-05`)
2. governance (`machine-10`,`machine-11`)
3. interop (`machine-06`..`machine-09`)
4. pqc (`machine-12`)
5. services (`machine-13`..`machine-15`)
...
This profile supports 11 node types. Each type has role-specific checks on node detail pages.

### 10.1 Consensus validators (`machine-01`..`machine-05`)
...
### 10.2 Interop relayer (`machine-06`)
...
```bash
cd ~/.synergy-node-monitor/monitor-workspace

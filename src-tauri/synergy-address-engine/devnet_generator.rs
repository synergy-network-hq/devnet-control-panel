// ============================================================================
// Synergy Devnet Key Generation Tool
...
fn write_genesis_file() {
    ...
    // Load validators
    for i in 1..=9 {
        let data = fs::read_to_string(format!("devnet/validator-{:02}/node_identity.json", i)).unwrap();
    ...
    // Load relayers
    for i in 1..=5 {
        let data = fs::read_to_string(format!("devnet/relayer-{:02}/node_identity.json", i)).unwrap();
        let ident: JsonIdentity = serde_json::from_str(&data).unwrap();
        relayers.push(json!({ 
            "address": ident.address,
    ...
    // Load RPC gateway
    let data = fs::read_to_string("devnet/rpc-gateway/node_identity.json").unwrap();
    let ident: JsonIdentity = serde_json::from_str(&data).unwrap();
    ...
    fs::write("devnet/genesis.json", serde_json::to_string_pretty(&genesis).unwrap()).unwrap();
...
fn write_node_configs() {
    ...
    for i in 1..=9 {
        ...
        fs::write(format!("devnet/validator-{:02}/node_config.toml", i), cfg).unwrap();
    }
    ...
    for i in 1..=5 {
        ...
        fs::write(format!("devnet/relayer-{:02}/node_config.toml", i), cfg).unwrap();
    }
    ...
    let cfg = "[node]\nrole='rpc-gateway'\nchain_id=338638\naddress_file='node_identity.json'\n\n\
               [network]\nrpc_port=8600\np2p_port=31400\nmetrics_port=9300\n\n\
               [cryptography]\nsignature_scheme='FN-DSA-1024'\n";
    fs::write("devnet/rpc-gateway/node_config.toml", cfg).unwrap();
    println!("✓ Created all node configs");
}

fn main() {
    println!("╔═══════════════════════════════════════════════════════════════╗");
    println!("║   Synergy Devnet FN-DSA-1024 Identity Generator              ║");
    ...
    println!("\n╔═══════════════════════════════════════════════════════════════╗");
    println!("║   Devnet generation complete!                                 ║");
